...
public class LeggiCodiceController 
{
    ...
    private void caricaDomande() 
    {
        eserciziPerLivello.put("Principiante", List.of(
            new Esercizio(titolo, Grado.PRINCIPIANTE, "int x = 5;\nint y = 3;\nint z = x + y;", "Qual è il valore di z?", new String[]{"8", "53", "x + y", "35"}, 0),
            new Esercizio(titolo, Grado.PRINCIPIANTE, "for(int i = 0; i < 3; i++) {\n  System.out.println(i);\n}", "Quante volte viene eseguito il ciclo?", new String[]{"3", "2", "4", "1"}, 0),
            new Esercizio(titolo, Grado.PRINCIPIANTE, "String s = \"Java\";\nSystem.out.println(s.length());", "Cosa viene stampato?", new String[]{"4", "Java", "3", "Errore"}, 0),
            new Esercizio(titolo, Grado.PRINCIPIANTE, "boolean b = true;\nif(!b) {\n  System.out.println(\"Vero\");\n} else {\n  System.out.println(\"Falso\");\n}", "Cosa viene stampato?", new String[]{"Falso", "Vero", "Niente", "Errore"}, 0),
            new Esercizio(titolo, Grado.PRINCIPIANTE, "int[] arr = {1,2,3};\nSystem.out.println(arr[1]);", "Cosa viene stampato?", new String[]{"2", "1", "3", "Errore"}, 0)
        ));

        eserciziPerLivello.put("Intermedio", List.of(
            new Esercizio(titolo, Grado.INTERMEDIO, "int x = 10;\nint y = x++;\nSystem.out.println(y);", "Cosa viene stampato?", new String[]{"10", "11", "12", "Errore"}, 0),
            new Esercizio(titolo, Grado.INTERMEDIO, "String s1 = \"Ciao\";\nString s2 = new String(\"Ciao\");\nSystem.out.println(s1 == s2);", "Cosa viene stampato?", new String[]{"false", "true", "Ciao", "Errore"}, 0),
            new Esercizio(titolo, Grado.INTERMEDIO, "List<Integer> list = new ArrayList<>();\nlist.add(1);\nlist.add(2);\nlist.remove(1);\nSystem.out.println(list.size());", "Cosa viene stampato?", new String[]{"1", "2", "0", "Errore"}, 0),
            new Esercizio(titolo, Grado.INTERMEDIO, "int x = 5;\nint y = x > 0 ? 1 : -1;\nSystem.out.println(y);", "Cosa viene stampato?", new String[]{"1", "-1", "5", "0"}, 0),
            new Esercizio(titolo, Grado.INTERMEDIO, "try {\n  int x = 5 / 0;\n} catch (Exception e) {\n  System.out.println(\"Errore\");\n}", "Cosa viene stampato?", new String[]{"Errore", "Niente", "0", "Infinity"}, 0)
        ));

        eserciziPerLivello.put("Avanzato", List.of(
            new Esercizio(titolo, Grado.AVANZATO, "Map<String, Integer> map = new HashMap<>();\nmap.put(\"A\", 1);\nmap.put(\"B\", 2);\nSystem.out.println(map.get(\"C\"));", "Cosa viene stampato?", new String[]{"null", "0", "Errore", "Niente"}, 0),
            new Esercizio(titolo, Grado.AVANZATO, "Supplier<Integer> supplier = () -> 5;\nSystem.out.println(supplier.get());", "Cosa viene stampato?", new String[]{"5", "supplier.get()", "Errore", "Niente"}, 0),
            new Esercizio(titolo, Grado.AVANZATO, "Stream<Integer> stream = Stream.of(1,2,3);\nlong count = stream.filter(n -> n > 1).count();\nSystem.out.println(count);", "Cosa viene stampato?", new String[]{"2", "3", "1", "0"}, 0),
            new Esercizio(titolo, Grado.AVANZATO, "Optional<String> opt = Optional.ofNullable(null);\nSystem.out.println(opt.orElse(\"Default\"));", "Cosa viene stampato?", new String[]{"Default", "null", "Niente", "Errore"}, 0),
            new Esercizio(titolo, Grado.AVANZATO, "List<String> list = Arrays.asList(\"A\",\"B\",\"C\");\nlist.forEach(System.out::print);", "Cosa viene stampato?", new String[]{"ABC", "A B C", "List[A,B,C]", "Errore"}, 0)
        ));
